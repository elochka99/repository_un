...
def rec_decompose(c):
    '''
    Функция подсчета количества различных представлений заданого числа. Рекурсивно беез повторений
    :param c: число
    :return: количество различных представлений числа 
    '''
    def func(c, id):
        if c == 0 and id == 0:
            return 1
...
while True:
    try:
        n = m = int(input('N = '))
        if n <= 0 or m <= 0:
            print('Не правильное число! введите число больше 0')
        else:
            kol = rec(n,m)
            kol1 = iter(n, m)
